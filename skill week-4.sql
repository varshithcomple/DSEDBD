...
SELECT Account_No, Account_Type, Balance, Branch FROM Account;
...
SELECT Account_No, Customer_ID, Account_Type, Balance FROM Account WHERE Balance > 100000;
SELECT * FROM High_Balance_View;
CREATE VIEW Hyderabad_Account_View AS
SELECT * FROM Account WHERE Branch = 'Hyderabad';
SELECT * FROM Hyderabad_Account_View;
CREATE VIEW Low_Balance_View AS
SELECT Account_No, Customer_ID, Balance FROM Account WHERE Balance < 50000;
...
CREATE OR REPLACE VIEW Hyderabad_Customer_Accounts AS
SELECT
C.Customer_Name,
C.City,
A.Account_No,
...
CREATE OR REPLACE VIEW Rich_Branches AS
SELECT Branch, SUM(Balance) AS Total_Balance FROM Account GROUP BY Branch HAVING SUM(Balance) > 100000;

SELECT * FROM Rich_Branches;
CREATE OR REPLACE VIEW Deposit_Transaction_View AS
SELECT * FROM Bank_Transaction WHERE Transaction_Type = 'DEPOSIT';
SELECT * FROM Deposit_Transaction_View;
CREATE OR REPLACE VIEW Withdrawal_Transaction_View AS
SELECT * FROM Bank_Transaction WHERE Transaction_Type = 'WITHDRAW';
...
CREATE OR REPLACE VIEW High_Value_Transaction_View AS
SELECT * FROM Bank_Transaction WHERE Amount > 10000;
SELECT * FROM High_Value_Transaction_View;
CREATE OR REPLACE VIEW Balance_Ranking_View AS
SELECT Account_No, Customer_ID, Account_Type, Balance FROM Account ORDER BY Balance DESC;
SELECT * FROM Balance_Ranking_View;
CREATE OR REPLACE VIEW Customer_Name_View AS
SELECT Customer_ID, Customer_Name, City FROM Customer ORDER BY Customer_Name;
SELECT * FROM Customer_Name_View;
CREATE OR REPLACE VIEW Loan_Interest_View AS
SELECT
Loan_ID,
Customer_ID,
Loan_Type,
...
CREATE OR REPLACE VIEW Loan_Total_Amount_View AS
SELECT
Loan_ID,
Customer_ID,
Loan_Type,
...
CREATE OR REPLACE VIEW High_Balance_View AS
SELECT Account_No, Customer_ID, Account_Type, Balance 
FROM Account 
WHERE Balance > 100000;
SELECT * FROM High_Balance_View WHERE Balance > 120000;
CREATE OR REPLACE VIEW Savings_Account_View AS
SELECT * 
FROM Account 
WHERE Account_Type = 'Savings';
SELECT * FROM Savings_Account_View WHERE Balance > 60000;
...
DROP VIEW Customer_Loan_View;
DROP VIEW IF EXISTS Deposit_Transaction_View, Withdrawal_Transaction_View;